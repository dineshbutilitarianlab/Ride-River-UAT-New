...
import BatchRecord from 'c/batchRecord';
import OrderProductLwc from 'c/orderProductLwcModal';
...
export default class CardsComponent extends LightningElement {
    ...
    // }
        ...
        debugger;
        console.log('Navigating to Batch Records');
    ...
    async navigateToAddMerchandiseOrders() {
        debugger;
        console.log('Navigating to Merchandise Records');
        const result = await OrderProductLwc.open({
            size: 'large',
            description: 'This is a modal popup for Add PO Payments', 
        });

        if (result === 'close') {
            console.log('Add Merchandise Order closed');
        }
    }

    async navigateToGetLeadDetails() {
        debugger;
        console.log('Navigating to Lead Records');
